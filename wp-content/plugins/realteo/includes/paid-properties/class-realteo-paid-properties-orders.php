		...
		foreach ( $order->get_items() as $item ) {
			if ( isset( $item['property_id'] )  ) {
				switch ( get_post_status( $item['property_id'] ) ) {
					case 'pending' :
						echo wpautop( sprintf( __( '%s has been submitted successfully and <b>will be visible once approved</b>. Visit <a href="//www.indigohomes.ph/my-properties">My Properties</a> to your property listing.', 'realteo' ), get_the_title( $item['property_id'] ) ) );
					break;
